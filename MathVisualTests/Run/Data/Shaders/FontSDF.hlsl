//------------------------------------------------------------------------------------------------
// Signed Distance Field (SDF) shader for Squirrel Eiserloh's C34 SD student Engine (Spring 2026)
// Custom shader for SDF (Tier 4+) font rendering and other SDF-based rendering effects.
// Requires Vertex_PCU vertex data (Position, Color, UVs).
// See Default.hlsl for explanation of naming conventions, etc.
//------------------------------------------------------------------------------------------------
...
float4 PixelMain( VertexOutPixelIn input ) : SV_Target0
{
	...
	if( finalColor.a <= 0.1 ) // a.k.a. "clip" in HLSL
	{
		discard;
	}
	else if( finalColor.a >= 0.7 )
	{
		finalColor = float4( 1,1,1,1 );
	}
	else
	{
		float alpha = (finalColor.a - 0.1) / (0.7 - 0.1);
		finalColor = float4( 1,0,0,alpha );
	}
